...
Zanima nas da li bračni status utiče na ishod s obzirom da on ima 208 vrednosti koje su prazni string.
```{r}
...
```{r}
ggplot(filter(data, na.rm = TRUE)) + geom_point(aes(x = Marital, y = Income))
```

Ni prihod ne utiče na bračni status. Ispitaćemo za BMI.
```{r}
ggplot(filter(data, na.rm = TRUE)) + geom_point(aes(x = Marital, y = BMI))
```
